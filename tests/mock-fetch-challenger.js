// mock-fetch-challenger.js
// Custom mock fetch preload for challenger tests to introduce delay and handle signal abort.
...
global.fetch = async (url, options) => {
  ...
  // Helper function to stream with delay and abort signal listening
  function createDelayedStream(chunks, signal) {
    return new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Stream might be aborted or closed
        } finally {
          if (signal) {
  ...
  if (urlStr.includes('api.openai.com/v1/chat/completions')) {
    console.log('[CHALLENGER MOCK FETCH] Intercepted OpenAI completions.');
    ...
    return new Response(createDelayedStream(chunks, signal), {
      status: 200,
  ...
  if (urlStr.includes('api.anthropic.com/v1/messages')) {
    console.log('[CHALLENGER MOCK FETCH] Intercepted Anthropic messages.');
    const chunks = [
  ...
  if (urlStr.includes('generativelanguage.googleapis.com')) {
    console.log('[CHALLENGER MOCK FETCH] Intercepted Google Gemini completions.');
    
    // Embeddings check:
    if (urlStr.includes(':embedContent')) {
      console.log('[CHALLENGER MOCK FETCH] Intercepted Google Gemini embedding.');
    ...
    if (urlStr.includes(':batchEmbedContents')) {
      console.log('[CHALLENGER MOCK FETCH] Intercepted Google Gemini batch embedding.');
      let numRequests = 1;
  ...
  if (urlStr.includes('api.openai.com/v1/audio/transcriptions')) {
    console.log('[CHALLENGER MOCK FETCH] Intercepted voice transcription API.');
